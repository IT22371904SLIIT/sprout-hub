  ...
  return (
    <div style={{ background: '#F9FBE7' }}>
      <div className='continer'>
        <NavBar />
        <div className='continSection' style={{ background: 'rgba(249, 251, 231, 0.7)' }}>
          ...
          <div className='post_card_continer'>
            {filteredPosts.length === 0 ? (
              <div className='not_found_box' style={{ color: '#8D6E63' }}>
              ...
              filteredPosts.map((post) => (
                <div key={post.id} className='post_card_new' style={{
                  ...
                  <div className={`template_dis template-${post.templateID}`} style={{
                    color: '#8D6E63',
                    borderColor: '#C5E1A5'
                  }}>
                    <div className='user_details_card'>
                      <div className='name_section_post'>
                        <p className='name_section_post_owner_name' style={{ color: '#4CAF50' }}>
                          {post.postOwnerName}
                        </p>
                      </div>
                      {post.postOwnerID === localStorage.getItem('userID') && (
                    ...
                    <p className='template_title' style={{ color: '#4CAF50' }}>{post.title}</p>
                    <p className='template_dates' style={{ color: '#8D6E63' }}>
                      <HiCalendarDateRange /> {post.startDate} to {post.endDate}
                    </p>
                    <p className='template_description' style={{ color: '#A1887F' }}>{post.category}</p>
                    <hr style={{ borderColor: '#C5E1A5' }}/>
                    <p className='template_description' style={{ color: '#8D6E63', whiteSpace: "pre-line" }}>
                      {post.description}
                    </p>
                    <div className="tags_preview">
                      {post.tags?.map((tag, index) => (
